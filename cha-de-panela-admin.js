...
const botaoAtualizarConvidados=document.getElementById('btn-atualizar-convidados-cha');
...
function atualizarInterfaceAdmin(){const conectado=Boolean(sessaoAdmin?.access_token);areaLogin.hidden=conectado;areaSessao.hidden=!conectado;emailLogado.textContent=conectado?(sessaoAdmin.user?.email||'administrador'):'';botaoAtualizar.disabled=!conectado;botaoAtualizarPresentes.disabled=!conectado;botaoAtualizarConvidados.disabled=!conectado;document.getElementById('btn-salvar-presente').disabled=!conectado;document.getElementById('btn-salvar-convidado-cha').disabled=!conectado;document.querySelectorAll('.btn-excluir-presente,.btn-editar-presente,.btn-convidado-cha,.admin-delete').forEach(botao=>botao.disabled=!conectado);if(!conectado){resumoRespostas.textContent='Entre para carregar as confirmações.';listaRespostas.innerHTML='<p>Entre para carregar.</p>';document.getElementById('lista-admin-presentes').innerHTML='<p>Entre para carregar.</p>';document.getElementById('lista-admin-convidados-cha').innerHTML='<p>Entre para carregar.</p>'}}
...
let convidadosChaAdmin=[];
function statusConvidadoChaAdmin(texto,erro=false){const area=document.getElementById('convidado-admin-status-cha');area.textContent=texto;area.classList.toggle('error',erro)}
async function carregarConvidadosChaAdmin(){const token=await tokenAdmin();if(!token)return;const area=document.getElementById('lista-admin-convidados-cha');botaoAtualizarConvidados.disabled=true;area.innerHTML='<p>Carregando...</p>';try{const resposta=await fetch(SUPABASE_URL+'/rest/v1/shower_guests?select=id,display_name,max_guests,active&order=display_name.asc',{headers:headersPublicos({Authorization:'Bearer '+token})});if(!resposta.ok)throw new Error('Não foi possível carregar a lista de convidados do chá.');convidadosChaAdmin=await resposta.json();area.innerHTML=convidadosChaAdmin.length?'':'<p>Nenhum convidado cadastrado no chá ainda.</p>';convidadosChaAdmin.forEach(item=>{const id=String(item.id);area.insertAdjacentHTML('beforeend',`<div class="item-admin"><div><strong>${escaparHtml(item.display_name)}</strong><small>Até ${item.max_guests} pessoa(s) · ${item.active?'ativo':'desativado'}</small></div><div class="item-admin-actions"><button class="btn-convidado-cha" type="button" onclick="alternarConvidadoCha('${id}',${!item.active})">${item.active?'Desativar':'Ativar'}</button><button class="btn-convidado-cha admin-delete" type="button" onclick="excluirConvidadoCha('${id}')">Excluir</button></div></div>`)});atualizarInterfaceAdmin()}catch(erro){area.innerHTML=`<p>${escaparHtml(erro.message)}</p>`;statusConvidadoChaAdmin(erro.message,true)}finally{botaoAtualizarConvidados.disabled=!sessaoAdmin}}
document.getElementById('form-add-convidado-cha').addEventListener('submit',async function(evento){evento.preventDefault();const token=await tokenAdmin();if(!token)return;const botao=document.getElementById('btn-salvar-convidado-cha');const nome=document.getElementById('convidado-nome-cha').value.trim();const limite=Number(document.getElementById('convidado-limite-cha').value);botao.disabled=true;statusConvidadoChaAdmin('Adicionando...');try{const resposta=await fetch(SUPABASE_URL+'/rest/v1/shower_guests',{method:'POST',headers:headersPublicos({Authorization:'Bearer '+token,'Content-Type':'application/json',Prefer:'return=minimal'}),body:JSON.stringify({display_name:nome,max_guests:limite})});const dados=await resposta.json().catch(()=>({}));if(!resposta.ok)throw new Error(dados.code==='23505'?'Este nome já está na lista do chá.':dados.message||'Não foi possível adicionar.');this.reset();document.getElementById('convidado-limite-cha').value=1;statusConvidadoChaAdmin('Convidado adicionado ao chá.');await carregarConvidadosChaAdmin()}catch(erro){statusConvidadoChaAdmin(erro.message,true)}finally{botao.disabled=!sessaoAdmin}});
async function alternarConvidadoCha(id,ativo){const token=await tokenAdmin();if(!token)return;const resposta=await fetch(SUPABASE_URL+'/rest/v1/shower_guests?id=eq.'+encodeURIComponent(id),{method:'PATCH',headers:headersPublicos({Authorization:'Bearer '+token,'Content-Type':'application/json'}),body:JSON.stringify({active:ativo,updated_at:new Date().toISOString()})});if(resposta.ok){statusConvidadoChaAdmin(ativo?'Convidado ativado.':'Convidado desativado.');await carregarConvidadosChaAdmin()}else statusConvidadoChaAdmin('Não foi possível alterar este convidado.',true)}
async function excluirConvidadoCha(id){const token=await tokenAdmin();const convidado=convidadosChaAdmin.find(item=>item.id===id);if(!token||!convidado||!confirm(`Excluir “${convidado.display_name}” e a confirmação do chá vinculada?`))return;statusConvidadoChaAdmin('Excluindo...');try{const respostaRsvp=await fetch(SUPABASE_URL+'/rest/v1/shower_guest_responses?guest_id=eq.'+encodeURIComponent(id),{method:'DELETE',headers:headersPublicos({Authorization:'Bearer '+token,Prefer:'return=minimal'})});if(!respostaRsvp.ok)throw new Error('Não foi possível excluir a confirmação vinculada.');const resposta=await fetch(SUPABASE_URL+'/rest/v1/shower_guests?id=eq.'+encodeURIComponent(id),{method:'DELETE',headers:headersPublicos({Authorization:'Bearer '+token,Prefer:'return=minimal'})});if(!resposta.ok)throw new Error('Não foi possível excluir o convidado.');statusConvidadoChaAdmin('Convidado e confirmação excluídos.');await Promise.all([carregarConvidadosChaAdmin(),carregarRespostasChaAdmin()])}catch(erro){statusConvidadoChaAdmin(erro.message||'Não foi possível excluir.',true)}}
...
async function carregarRespostasChaAdmin(){
  ...
  try{
    const resposta=await fetch(SUPABASE_URL+'/rest/v1/shower_guest_responses?select=id,guest_id,guest_name,attending,guest_count,updated_at&order=updated_at.desc',{headers:headersPublicos({Authorization:'Bearer '+token})});
...
document.getElementById('form-admin-login-cha').addEventListener('submit',async function(evento){
  ...
  try{
    ...
    await Promise.all([carregarPresentesAdmin(),carregarConvidadosChaAdmin(),carregarRespostasChaAdmin()]);
...
botaoAtualizarConvidados.addEventListener('click',carregarConvidadosChaAdmin);
